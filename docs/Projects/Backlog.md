...
- **Dostępne nazwy modali panelu — dług wzorca** (2026-08-31, z przeglądu E4): wzorzec `wpd-dialog` nie nadaje dialogom `aria-labelledby` ani nazwy przyciskowi zamknięcia (ikona X bez `aria-label`) — czytnik ekranu ogłasza bezimienny „dialog" i bezimienny „przycisk". PatchNotesModal dostał oba atrybuty lokalnie (jako jedyny otwiera się sam), ale DailyReportModal, SettingsModal, AccountModal i pozostałe dzielą ten brak. **Po co:** naprawa raz we wzorcu (np. `useDialogA11y` przyjmujący id nagłówka) zamyka to we wszystkich modalach naraz. Sprzężone z D2 (przegląd dostępności panelu administratora — ta sama klasa braków).
