...
| **E3** | **Moduł Zadania (szósty widok panelu)** — zakres rozstrzygnięty 2026-08-28: **bez eksportu** (integracja z kalendarzem telefonu wydzielona i zamknięta jako E6). Design hi-fi dostarczony przez właściciela 2026-08-28: **`_design-reference/design_handoff_zadania/`** (⚠️ wcześniejszy wskaźnik na `design_handoff_identyfikacja_v2` był błędny — tam projektu zakładki nie ma). Zakres wg handoffu: kolekcja `tasks`, lista dzienna, oś przypisania z przeciąganiem, popover szybkiego zadania; partia 1 = kroki 1–5 planu, materializacja szablonów/zdjęcia/migracja = partia 2 | `dev` + `designer` + `code-reviewer` | ✅ **PARTIA 1 GOTOWA W KODZIE 2026-08-29** (kroki 1–5: kolekcja `tasks` z regułami/indeksami/lustrem, widok 06 z osią i przeciąganiem + ścieżka klawiaturowa; przegląd `code-reviewer` + 7 poprawek; bramki: e2e 228/228, functions 65/65 z 16 testami lustra, podatki 37/37, lint 0). ⏳ **deploy reguł+indeksów+functions+hosting za zgodą właściciela** (bramka `reguly`: tester na produkcji z kluczem serwisowym, diff konsola↔repo, osobny cel indeksów) · **decyzja właściciela:** partia 2 przed pokazaniem becie (nowe zadania nie są jeszcze widoczne w Raporcie dziennym/widgecie/szczegółach rezerwacji — okno zgodności działa w jedną stronę) · okno roku → [[Known-Issues]] #21 |
