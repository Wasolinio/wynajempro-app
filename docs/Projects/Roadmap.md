...
| **E4** | **System popupowych patch notów** — komunikat „co nowego" po wdrożeniu zmian | `dev` + `designer` | ✅ **KOD GOTOWY I PRZEJRZANY 2026-08-31** (`5b26312`): oba pytania rozstrzygnięte — treść z `docs/marketing/patch-notes.md` (markdown = źródło prawdy, `npm run patchnotes:build`, bundlowana z aplikacją → nota wychodzi dokładnie z deployem, którego dotyczy), „widziane" per konto polem `lastSeenPatchNote` w `users/{uid}` **bez zmiany reguł** (+ zapasowy localStorage dla kont historycznych bez pola `status`, którym reguła odbija update — finding przeglądu). Świeżo zarejestrowany popupu nie dostaje (odcięcie po dacie rejestracji). Na start 5 wpisów spójnych z serią grafik. Bramki: lint 0, build OK, e2e 240/240 (patch-notes 6/6), przegląd `code-reviewer` naniesiony. ✅ **WYDANE NA PRODUKCJĘ 2026-08-31** (`hosting:app`, za zgodą właściciela): weryfikacja live zaliczona — nowy hash serwowany po aktywacji SW, trasy 200, 301 zachowuje ścieżkę, konsola czysta, a chunki pobrane z produkcji niosą treść wpisów i obie poprawki po przeglądzie. ⏳ **zostaje smoke właściciela w panelu** (popup wymaga zalogowanego konta): czy okienko się pokazuje, czy „Rozumiem" je zamyka i czy po ponownym wejściu nie wraca. Szczegóły: [[Activity-Log]] 2026-08-31 |
