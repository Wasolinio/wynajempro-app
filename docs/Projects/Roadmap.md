...
| **E6** | **„Dodaj do kalendarza" przy zadaniu (wzorzec Booksy)** — decyzja właściciela 2026-08-28, doprecyzowana tego samego dnia: **nie kanał subskrypcyjny, lecz przycisk** — klik zapisuje zadanie jako wydarzenie w kalendarzu telefonu, przypomnienie ustawia aplikacja kalendarza. **Po co:** zadanie ma przypominać o sobie tam, gdzie gospodarz patrzy — w telefonie. **Gotowe, gdy:** przy zadaniu działa przycisk generujący wydarzenie (Google Calendar / plik .ics), termin liczy wyłącznie `src/utils/taskSchedule.js`. **Weryfikacja:** e2e na treści wygenerowanego zdarzenia + test ręczny właściciela na telefonie. Rezerwacje mają już własny kanał (`exportIcal`) — E6 ich nie dotyczy | `dev` + przegląd `code-reviewer` | ✅ **WYDANE NA PRODUKCJĘ 2026-08-28** (commit `af7374a`; przegląd „bezpieczne do commita", e2e 211/211; deploy `hosting:app` za zgodą właściciela, weryfikacja live zaliczona — chunk z funkcją serwowany, trasy, 301, konsola czysta poza App Check 403). iOS wymagał trzech rund (testy właściciela na urządzeniu obaliły kolejno pobieranie i blob-nawigację): runda 3 (`83ac590`, trzeci deploy — functions `taskIcs` + hosting) przeszła na wzorzec Booksy — nawigacja do https z `text/calendar` inline; strzał kontrolny endpointu zaliczony, e2e 213/213, functions 49/49. ⏳ zostaje: trzeci test na iPhonie (Safari + PWA standalone) — podgląd „Dodaj wszystkie", alarm 9:00, podwójny import. Szczegóły: [[Activity-Log]] 2026-08-28 |
