...
**Weryfikacja:** per-pozycja niżej. **Agent:** `dev` + `legal` (brzmienia) + właściciel (konsola/decyzje). **Status:** ⬜ nowe (2026-07-23, z oceny 3 dokumentów dla prawnika)
- **N6.1 Wycofanie zgody na cookies (art. 7 ust. 3 RODO)** — 🔄 KOD GOTOWY (2026-07-23, `dev`). Mechanizm: `src/firebase.js` — `disableAnalytics()` (`ga-disable-<ID>=true` + `setAnalyticsCollectionEnabled(false)` + kasowanie ciastek `_ga*`) / `enableAnalytics()`; `initAnalytics()` utwardzony — **nie tworzy GA bez zapisanej zgody** (domyka bonusową lukę: zdarzenia z `LoginPanel` leciały do GA przed zgodą). Wejścia „równie łatwo": link „Ustawienia cookies" w stopce landingu + przycisk „Zmień/wycofaj zgodę" na stronie Polityki → otwierają globalny `ConsentNotice` (zdarzenie `wpc:open`, bez przeładowania), z linią statusu i symetrycznym accept/withdraw. Weryfikacja: lint 0, build OK, nowy `e2e/cookie-consent.spec.js` 3/3, podgląd wizualny (flaga `ga-disable-*` przełącza się). **Brzmienie domknięte przez `legal` (2026-07-24):** 4/5 tekstów OK; treść banera skorygowana (usunięto nadmiarowe „dostosowywać komunikaty" — aplikacja nie personalizuje, po zgodzie działa tylko GA; poprawił `dev`, e2e 3/3). Dokumenty zaktualizowane uczciwie („kod gotowy, oczekuje na deploy", nie „live"): `Polityka-prywatnosci.md` §9 + `Bezpieczenstwo-kont-i-danych.md` §9. **Zostaje:** (1) commit/deploy N6.1+N6.2 — `firebase deploy --only hosting:app` (decyzja właściciela); po deployu `legal` zdejmuje „oczekuje na deploy" z §9. (2) *Wsad do N4 (prawnik):* podstawa cookies „art. 173 Pr. tel." nieaktualna → PKE art. 399/402 (od 2024-11-10) — dotyczy Polityki, Checklisty i Oceny-linki; `legal` oznaczył w dokumentach. (3) *Opcjonalnie (`designer`):* wizualna równowaga przycisków banera (primary „Akceptuję" vs ghost „Wycofaj") pod kątem „równej łatwości" — do decyzji.
- **N6.2 Ostrzeżenie przy usuwaniu konta — pełny zakres kasacji** — 🔄 KOD GOTOWY (2026-07-23, `dev`). Stan wyjściowy: `AccountModal.jsx:182` obiecywał mniej („konto, subskrypcja, nieruchomości, historia rezerwacji") niż realnie kasuje `deleteUserAccount`. Komunikat przepisany na pełny zakres (przewodniki + sekrety WiFi/PIN + podpisy akceptacji, pliki, dane biznesowe, rekord klienta Stripe, profil, konto logowania) + „nieodwracalne, bez karencji" — wprost z §8. Weryfikacja: lint 0, build OK; grep e2e — brak asercji na tym copy. **Zostaje:** commit/deploy (decyzja właściciela) + zerknięcie `legal` na brzmienie; po deployu zaktualizować wiersz §9 dokumentu bezpieczeństwa. Zmiany w working tree, niecommitowane.
- **N6.3 Kopie zapasowe Firestore** — ⏸ decyzja + konsola właściciela (waga średnia–wysoka; §9: „rekomendowane przed launchem"). Redundancja Google **nie chroni przed omyłkowym skasowaniem danych przez aplikację**. **Gotowe, gdy:** włączony PITR lub harmonogram eksportu do bucketu + wpis w [[Zlecenia-wlasciciela]]. **Agent:** właściciel (Firebase/GCP). → [[Zlecenia-wlasciciela]] #8 pkt 2.
...
5. **Braki bezpieczeństwa z §9 do rozstrzygnięcia** (nie blokują startu, ale wymagają decyzji — z oceny 2026-07-23, powiązane z N6):
   - **Polityka haseł** — podnieść z domyślnych 6 znaków (złożoność / sprawdzanie haseł z wycieków); konfiguracja w konsoli Firebase Authentication. Waga niska–średnia, tanie.
