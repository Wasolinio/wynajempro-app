...
| **B2** | ✅ **DECYZJA WŁAŚCICIELA 2026-08-27**: konta po „Odbierz" — retencja **90 dni** (spójnie z retencją porzuconych triali z Polityki §2 i Regulaminu §11 ust. 2); konta z nadanym dostępem — **bez automatu do końca bety**, ręczny przegląd w raporcie „Porządek". Bramka „przed pierwszym testerem" zdjęta | ⏳ ogon (`dev`, nie blokuje zaproszeń — pierwsze konto po „Odbierz" osiągnie termin najwcześniej pod koniec listopada): mechanizm kasowania po „Odbierz", najlepiej przez istniejącą ścieżkę porzuconych triali (`trialEndsAt`), żeby obecna treść Polityki pokrywała bez zmian |
| **B3** | ✅ **NANIESIONE 2026-08-26** — wiersz `admin_audit` w Polityce §2: zakres, cel, podstawa (art. 6 ust. 1 lit. f), okres 12 mies. Zakres spisany z kodu, nie z projektu w ocenie — `audit()` celowo nie zapisuje e-maila, więc wiersz wyszedł **węższy** niż projekt. Do §6 dopisane, że dziennik przeżywa usunięcie Konta | ⏸ zostaje decyzja właściciela o okresie (12 vs 24 mies.) i potwierdzenie podstawy przez prawnika |
| **B4** | ✅ **NANIESIONE 2026-08-26** — wiersz `contact_messages` rozszerzony o pola obsługi: status (nowe/w toku/zamknięte), notatka do 5000 znaków, data zmiany, identyfikator osoby działającej | ⏸ zostaje decyzja o okresie przechowywania zgłoszeń (propozycja 12 mies.) |
| **B5** | Warunki founding members w Regulaminie §6 — **w większości USTALONE** ([[Decisions]] ADR-016, 2026-08-22: cena startowa gwarantowana 12 mies., zmiana ceny z 30-dniowym wyprzedzeniem, bez kodu rabatowego). ✅ **DOMKNIĘTE 2026-08-26**: zamiast daty — konstrukcja zdarzeniowa (nabór trwa do ogłoszenia zakończenia z 14-dniowym wyprzedzeniem w trybie §10; wariant 2 z analizy, decyzja właściciela). Pętla ze ścieżką A rozcięta — §6 jest kompletny bez daty | — |
...
### Ścieżka F — publikacja dokumentów (nowa, 2026-08-26)

> Powstała z ustalenia, że **wypełnienie placeholderów nie wystarczy**: strony na produkcji mają
> własną, starszą treść i nie czytają z `docs/legal/`. Pełny inwentarz luk i podział
> odpowiedzialności: `docs/legal/Bramka-publikacji-2026-08-26.md`.

| # | Co | Kto | Zależność / uwaga |
|---|---|---|---|
| **F1** | ✅ **ZAMKNIĘTE 2026-08-26** — właściciel podał komplet: Szymon Wasiak, Nowogródzka 9/9, 82-300 Elbląg, kontakt@wynajempro.com. Naniesione we wszystkie miejsca trzech dokumentów (8 luk zamkniętych, stan bramki: 35) | właściciel | — |
| **F2** | ✅ **ZAMKNIĘTE 2026-08-26** — właściciel zatwierdził wszystkie 11 decyzji po propozycjach; naniesione (stan bramki: 21 luk) | właściciel | — |
| **F7** | ✅ **WYDANE 2026-08-26** — **purge `contact_messages` po 12 mies. od zakończenia korespondencji** (decyzja B-5): scheduler `cleanupContactMessages` (codziennie 03:45), koniec korespondencji = `max(createdAt, adminUpdatedAt)`. Commit `85bcb47`, merge + `firebase deploy --only functions` za zgodą właściciela; `functions:list` potwierdza trigger `scheduled`. Bramki: testy 41/41, e2e 204/204, przegląd `code-reviewer` (logika potwierdzona; finding o kłamiącej notce panelu naprawiony w `222c656`). 🔴 warunek publikacji Polityki **zszedł**. Szczegóły: [[Activity-Log]] 2026-08-26 | `dev` | ✅ ogon domknięty przy publikacji: wieczorny deploy `hosting:app` 2026-08-26 (wydanie poprawkowe) budował z `main` zawierającego już `222c656` (potwierdzone `git merge-base --is-ancestor`, 2026-08-27) — panel pokazuje aktualną notkę |
| **F3** | **VAT, faktury, retencja dokumentów księgowych** | księgowy | 3 luki; zależne od formy działalności (nierejestrowana vs JDG) |
| **F4a** | ✅ **WYKONANE 2026-08-26 przy publikacji** — poziom 3 supportu zniesiony (proces, skill `zgloszenie`, CLAUDE.md przepisane na stan ostateczny); DPA prawdziwe od pierwszego dnia obowiązywania | agent | — |
| **F4** | ✅ **WYDANE NA PRODUKCJĘ 2026-08-26** (commit `328cd71`, `hosting:app`) — `/regulamin`, `/prywatnosc` i nowa `/dpa` renderują treść z `docs/legal/` przez generator z filtrem publikacyjnym; Obowiązuje od: 26 sierpnia 2026 r.; weryfikacja live zaliczona (trasy, treść, przycisk zgody, 301, konsola czysta poza App Check 403). Zmiany treści od dziś: edycja `docs/legal/` → `npm run legal:build` → deploy | `dev` | — |
| **F5** | ✅ **ZAMKNIĘTE INACZEJ 2026-08-26** — właściciel zrezygnował z prawnika; 9 pytań rozstrzygnięte analizą własną (`docs/legal/Analiza-prawna-2026-08-26.md`, ADR-025). Ogon po decyzjach z 2026-08-26: Anthropic wykreślony z DPA (poziom 3 do końca bety — patrz F4a); plan Anthropic wciąż warunkuje wpis w **Polityce §5** (zgłoszenia i dane konta); oświadczenie ADR w każdej odpowiedzi na reklamację (art. 32 — milczenie oznacza zgodę) | agent + właściciel | — |
| **F6** | ✅ **ZAMKNIĘTE 2026-08-26** — wszystkie 6 ustalone u źródła (region eur3, mechanizmy transferu Google/Stripe/Anthropic, retencje logów i GA); wyniki w analizie | agent | — |

⚠️ **Kolejność:** F1 przed wszystkim; F3 i F4 mogą iść równolegle, bo nie zależą od F1.
...
| **C2** | Eksport banerów social — 13 formatów (IG post/story, FB cover/post, LinkedIn cover/post, „nowa funkcja", Google Ads ×3, OG danymi). ✅ **ZAMKNIĘTE 2026-08-31** (`ffd6785`): komplet w `docs/marketing/banery/` + generator `npm run banery:build` (`scripts/build-social-banners.mjs` — renderuje też odcinki serii, domyka dziurę po skrypcie ze scratchpada z 28.08). Po drodze: **cały dokument identyfikacji miał stary znak (trzy paski)** — generator podmienia na monogram W w locie (35 podmian), źródła serii poprawione (grafiki z posta FB 28.08 niosły stary znak); nazewnictwo X24 w dwóch banerach LinkedIn; obietnica „koniec z overbookingiem" przycięta do mechanizmu co godzinę (wzorzec X26); **bloker arytmetyczny grafiki podatkowej serii** (19% liczyło się z niewidocznej podstawy — dochód po kosztach jest teraz jawnie na karcie). Weryfikacja: przegląd 13 agentów vs spec sekcji 10–16 + wizualnie wszystkie PNG. Świadome odstępstwa spisane w nagłówku skryptu; pułapki użycia (m.in. fb-cover a pole bezpieczne mobile) w `docs/marketing/banery/README.md`. 🔄 **START 2026-08-28**: szablon „nowa funkcja" (`87eafd1`, agent `designer`) | agent `designer` |
...
| # | Co | Uwaga |
|---|---|---|
| **D1** | ✅ **PRZEJŚCIE LISTY KONTROLNEJ WYKONANE 2026-08-25** — 5 punktów zaliczonych, 6. świadomie skreślony przez właściciela („nie będę korzystał z panelu na telefonie"; sprawdzanie ekranu, z którego się nie korzysta, to praca bez pokrycia — **nie wracać do tego przy kolejnych przeglądach**). Wynik: mechanizm zapraszania testerów **potwierdzony**, nie „prawdopodobny". 🛑 Po drodze dwa znaleziska: [[Known-Issues]] **#19** (MRR liczy konta nadane ręcznie) i myląca stopka w Zgłoszeniach (naprawiona od razu) |
...
| **E3** | **Moduł Zadania (szósty widok panelu)** — zakres rozstrzygnięty 2026-08-28: **bez eksportu** (integracja z kalendarzem telefonu wydzielona i zamknięta jako E6). Design hi-fi dostarczony przez właściciela 2026-08-28: **`_design-reference/design_handoff_zadania/`** (⚠️ wcześniejszy wskaźnik na `design_handoff_identyfikacja_v2` był błędny — tam projektu zakładki nie ma). Zakres wg handoffu: kolekcja `tasks`, lista dzienna, oś przypisania z przeciąganiem, popover szybkiego zadania; partia 1 = kroki 1–5 planu, materializacja szablonów/zdjęcia/migracja = partia 2 | `dev` + `designer` + `code-reviewer` | ✅ **PARTIE 1 I 2 GOTOWE W KODZIE 2026-08-29** — partia 1 (`8802804`, kroki 1–5: kolekcja `tasks` z regułami/indeksami/lustrem, widok 06 z osią, przeciąganie + ścieżka klawiaturowa) i partia 2 (`f594fbb`, kroki 6–8: wspólne źródło zadań w całym panelu, materializacja szablonów, powtarzalność, mobile < 980 px, zdjęcia w Storage, skrypt migracji). Oba przeglądy wykonane; w partii 2 **znalezisko blokujące** — migracja kasowała drugie z dwóch identycznych zadań bez kopii, a dry-run tego nie pokazywał ([[Activity-Log]] 2026-08-29). Bramki potwierdzone własnym przebiegiem: e2e 233/233, functions 73/73, podatki 37/37, lint 0, build OK. ✅ **WYDANE NA PRODUKCJĘ 2026-08-29** — pięć celów (`firestore:rules` + `firestore:indexes` → `storage` → `functions` → `hosting:app`; `functions` doszło, bo czyszczenie konta objęło `tasks` i prefiks Storage — RODO art. 17). Bramka `reguly` w komplecie: tester na produkcji **51/51 dokumentów**, diff konsola↔repo **identyczny** (deploy = same dodania), kontrola po wydaniu **potwierdzona odczytem z produkcji**. Weryfikacja live przez przeglądarkę zaliczona. 🔥 **Migracja bezprzedmiotowa**: produkcja ma **zero** wpisów `type:'reminder'` (potwierdzone niezależnym przeliczeniem: 4 konta, 28 dokumentów) — skrypt czeka nieuruchomiony. ✅ **SMOKE WŁAŚCICIELA ZALICZONY 2026-08-29** („wszystko działa zajebiście") — zadania, przeciąganie, zdjęcia i usuwanie przeklikane na produkcji; reguły potwierdzone od strony klienta. 🛑 Po wydaniu wyszła jedna luka: **usuwania zadań nie było w PROJEKCIE** (ani README handoffu, ani prototyp) — dodane i wydane tego samego dnia (`fc6ebcd`), przy okazji złapana i naprawiona u źródła regresja chwytu kartki w skrzynce. **E3 ZAMKNIĘTE W CAŁOŚCI.** ⏸ **Do decyzji z `legal`**: prywatność zdjęć opiera się na nieodgadywalnym tokenie, nie na regule `read` · okno roku → [[Known-Issues]] #21 |
| **E4** | **System popupowych patch notów** — komunikat „co nowego" po wdrożeniu zmian | `dev` + `designer` | ✅ **KOD GOTOWY I PRZEJRZANY 2026-08-31** (`5b26312`): oba pytania rozstrzygnięte — treść z `docs/marketing/patch-notes.md` (markdown = źródło prawdy, `npm run patchnotes:build`, bundlowana z aplikacją → nota wychodzi dokładnie z deployem, którego dotyczy), „widziane" per konto polem `lastSeenPatchNote` w `users/{uid}` **bez zmiany reguł** (+ zapasowy localStorage dla kont historycznych bez pola `status`, którym reguła odbija update — finding przeglądu). Świeżo zarejestrowany popupu nie dostaje (odcięcie po dacie rejestracji). Na start 5 wpisów spójnych z serią grafik. Bramki: lint 0, build OK, e2e 240/240 (patch-notes 6/6), przegląd `code-reviewer` naniesiony. ✅ **WYDANE NA PRODUKCJĘ 2026-08-31** (`hosting:app`, za zgodą właściciela): weryfikacja live zaliczona — nowy hash serwowany po aktywacji SW, trasy 200, 301 zachowuje ścieżkę, konsola czysta, a chunki pobrane z produkcji niosą treść wpisów i obie poprawki po przeglądzie. ✅ **SMOKE WŁAŚCICIELA ZALICZONY 2026-08-31** („wszystko działa, popup nie wraca") — zapis `lastSeenPatchNote` potwierdzony od strony klienta. **E4 ZAMKNIĘTE W CAŁOŚCI.** Kolejne wpisy: edycja `docs/marketing/patch-notes.md` → `npm run patchnotes:build` → zwykły deploy. Szczegóły: [[Activity-Log]] 2026-08-31 |
...
| **E6** | **„Dodaj do kalendarza" przy zadaniu (wzorzec Booksy)** — decyzja właściciela 2026-08-28, doprecyzowana tego samego dnia: **nie kanał subskrypcyjny, lecz przycisk** — klik zapisuje zadanie jako wydarzenie w kalendarzu telefonu, przypomnienie ustawia aplikacja kalendarza. **Po co:** zadanie ma przypominać o sobie tam, gdzie gospodarz patrzy — w telefonie. **Gotowe, gdy:** przy zadaniu działa przycisk generujący wydarzenie (Google Calendar / plik .ics), termin liczy wyłącznie `src/utils/taskSchedule.js`. **Weryfikacja:** e2e na treści wygenerowanego zdarzenia + test ręczny właściciela na telefonie. Rezerwacje mają już własny kanał (`exportIcal`) — E6 ich nie dotyczy | `dev` + przegląd `code-reviewer` | ✅ **WYDANE NA PRODUKCJĘ 2026-08-28** (commit `af7374a`; przegląd „bezpieczne do commita", e2e 211/211; deploy `hosting:app` za zgodą właściciela, weryfikacja live zaliczona — chunk z funkcją serwowany, trasy, 301, konsola czysta poza App Check 403). ✅ **ZAMKNIĘTE W CAŁOŚCI 2026-08-28** — potwierdzenie właściciela na iPhonie („Wszystko działa super!"). iOS wymagał trzech rund (testy na urządzeniu obaliły kolejno pobieranie i blob-nawigację); ostatecznie wzorzec Booksy: nawigacja do Cloud Function `taskIcs` (https + `text/calendar` inline), `83ac590`, e2e 213/213, functions 49/49. Ogony poza E6: flagi RODO dla `legal` (nazwisko gościa w URL Google i w logach żądań taskIcs), a11y menu w [[Projects/Backlog]]. Szczegóły: [[Activity-Log]] 2026-08-28 |
| **E7** | **Próg zwolnienia podmiotowego VAT + przegląd form opodatkowania** — zlecenie właściciela 2026-08-28 po prześwietleniu panelu pod zarzuty z naboru. **Po co:** panel ma udźwignąć zarzuty z komentarzy („gdzie VAT/ZUS/import usług") i objąć jak najszersze pole klientów. **Gotowe, gdy:** karta limitu 240 000 zł (art. 113) w panelu i eksporcie + raport form z rekomendacjami per forma. **Weryfikacja:** złote testy liczone ręcznie + przegląd `code-reviewer` z ręcznym przeliczeniem | `legal` + `dev` + `code-reviewer` | ✅ **WYDANE NA PRODUKCJĘ 2026-08-28** — jedno wydanie `hosting:app`: próg VAT 240 tys. (`39f2795`, ADR-026) + **liniowy 19%** (`713f46a`, ADR-027; decyzja właściciela „wdróż już teraz" nadpisała rekomendację „przy popycie"). Bramki: podatki 37/37, e2e 218/218, przegląd z ręcznym przeliczeniem; weryfikacja live zaliczona. Raport form: `docs/legal/Formy-opodatkowania-wynajmu-2026-08-28.md` (części I–III). ⏳ przegląd brzmień przez prawnika/doradcę przy następnej rewizji (ADR-026/027) |
| **E8** | **Humanizacja landingu (feedback „AI slop")** — zlecenie właściciela 2026-09-03; plan 6 kroków z [[strategy/Analiza-konkurencji-Smoobu-i-rynek-2026-08-31]] §7. **Po co:** strona bez żadnego artefaktu rzeczywistości czyta się jako wygenerowana, a beta bez zaufania nie rekrutuje. **Gotowe, gdy:** makiety zastąpione zrzutami prawdziwego panelu na jawnie przykładowych danych, tiki generatora zdjęte, każde twierdzenie ma pokrycie w kodzie, publiczny changelog działa, a materiały właściciela (zdjęcia, założyciel, liczby z bety) wchodzą warunkowo — bez atrap. **Weryfikacja:** e2e (`landing-demo`, `co-nowego`, `spelling`, `ui-scaling`), podgląd 1280/375, przegląd `code-reviewer`, weryfikacja live po deployu | `dev` + `designer` + `code-reviewer`; materiały: właściciel | ✅ **KOD GOTOWY 2026-09-03** (część A): generator zrzutów `npm run zrzuty:build` (`scripts/zrzuty/`, poza CI), scroll-demo na 4 zrzutach, landing bez tików i bez twierdzeń bez pokrycia, `/co-nowego`, daty z danych w stopce, warunkowe bloki `landingProof.js`; lint 0, build OK, e2e 244/244 — szczegóły [[Activity-Log]] 2026-09-03. ✅ **CZĘŚĆ A WYDANA NA PRODUKCJĘ 2026-09-15** (`f6ef3e2`, `hosting:app`, 49 plików; zgoda właściciela „Działaj po kolei tak jak uważasz"): weryfikacja live zaliczona — trasy 200, cztery zrzuty serwowane, `/co-nowego` z 5 wpisami i canonical, 301 trzyma ścieżkę, konsola czysta; e2e 244/244, smoke produkcji 9/9 z nowym testem `/co-nowego`. Szczegóły [[Activity-Log]] 2026-09-15. 🔄 **Decyzje (a)–(g) podjęte 2026-09-17** (a NIE · b NIE · c TAK · d TAK · e TAK · f zostaje · g NIE): karta „Z praktyki" w pierwszej osobie z nazwą „Domki Letniskowe Ruś" i FAQ „odpisuję osobiście" ✅ **WYDANE NA PRODUKCJĘ 2026-09-17** (`ffc22c0`, `hosting:app`; zgoda „tak, wdrażaj"; e2e 244/244, weryfikacja live i smoke produkcji 9/9 zaliczone); blok założyciela i ocena Airbnb **nie powstają** (decyzja, zapis w `landingProof.js`). ✅ **Zdjęcie domku (własny kadr z drona) WYDANE NA PRODUKCJĘ 2026-09-17** (`0fbc5ad`; `npm run zdjecia:build`, 223 kB; podpis „Warmia i Mazury", dopisek właściciela z terminem „wiosna 2027"; e2e 244/244, weryfikacja live i smoke 9/9). ⏸ **Z części B zostają liczby z `/admin`** — zlecenie **#15** ([[Projects/Zlecenia-wlasciciela]]): zdjęcia domków, zdjęcie twarzy, decyzje (a)–(g), dwa zdania, liczby z `/admin`. Iteracja 2 w [[Projects/Backlog]] |
...
4. **E8** — pakiet decyzji (a)–(g) do uwiarygodnienia landingu (zlecenie #15): nazwisko i wizerunek na stronie, nazwa „Domki Letniskowe Ruś" i precyzja miejsca, „odpisuję osobiście" w FAQ, pierwsza osoba w karcie „Z praktyki", zakres „1–20 obiektów", ocena Airbnb 5,0 (rekomendacja: jeszcze nie). Bez nich część B nie wchodzi; część A nie czeka.
...
### N4. Regulamin, polityka prywatności, powierzenie danych (DPA) — ✅ AKCEPTACJA PRAWNIKA 2026-08-25
...
**Agent:** `legal` (projekty) → właściciel + prawnik (akceptacja) → `dev` (wdrożenie). **Status:** ✅ **PRAWNIK ODPOWIEDZIAŁ 2026-08-25 — BEZ ZASTRZEŻEŃ** do aktualnego stanu dokumentów. Zaproponował **kolejną rewizję po przeskalowaniu biznesu**, przy większej liczbie klientów — czyli rewizja jest wyzwalana **zdarzeniem, nie datą**; zapisana w [[Projects/Backlog]].
- 🎯 **Co to zamyka:** N4 był **ostatnim otwartym blokerem launchu** (patrz N6, domknięte 2026-08-18). Po stronie merytorycznej dokumentów nie ma już nic do zrobienia.
- 🔴 **Czego to NIE zamyka — bramka publikacji stoi dalej i to nie jest formalność.** W dokumentach zostają **43 luki** (Regulamin 16, Polityka 17, DPA 10). ⚠️ **Korekta 2026-08-26:** wcześniejsze „36” liczyło **wiersze** z napisem „DO UZUPEŁNIENIA”, nie luki — pomijało drugie wystąpienie w wierszu oraz warianty `[DO DECYZJI]` i `[DO POTWIERDZENIA]`. Faktyczny stan przed 2026-08-26 to **47**; cztery zamknęło potwierdzenie adresu `kontakt@wynajempro.com`. Pełny inwentarz z podziałem odpowiedzialności: `docs/legal/Bramka-publikacji-2026-08-26.md`. Najważniejsze z nich to **tożsamość Operatora** (imię i nazwisko, adres do korespondencji, e-mail kontaktowy) — obowiązek z UŚUDE art. 5 i RODO art. 13, **niezależny od formy działalności**, więc działalność nierejestrowana go nie znosi. Do tego adres do reklamacji i odstąpienia oraz sposób udostępniania faktur. **Dokumentu z „[DO UZUPEŁNIENIA]" nie wolno opublikować** — akceptacja prawnika dotyczy treści, nie luk w niej.
- ⚖️ **Uwaga na zakres akceptacji:** prawnik ocenił dokumenty w brzmieniu z pakietu. Jeśli placeholdery zostaną wypełnione treścią zmieniającą sens (np. inny model faktur niż zakładany), akceptacja tego nie obejmuje.
- ⛔ **Generator umów najmu NIE wraca — decyzja właściciela 2026-08-25.** Schowano go z nawigacji 2026-07-16 do czasu akceptacji prawnej; akceptacja przyszła, warunek odpadł, a właściciel mimo to zostawia go ukrytego. Kod zostaje w repo (`e211c8d`), więc przywrócenie to nadal jedna zmiana w nawigacji, gdyby decyzja się odwróciła. **Agent nie wraca do tego z własnej inicjatywy.**
...
6. **Materiały i decyzje do landingu (E8, zlecenie #15, 2026-09-03)** — zdjęcia domków, zdjęcie twarzy, decyzje (a)–(g), dwa zdania „dlaczego to zbudowałem", liczby z `/admin`; instrukcja w [[Projects/Instrukcje-wlasciciela]] sekcja 10.
