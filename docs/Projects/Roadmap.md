...
| **E3** | **Nowa zakładka w panelu: Zadania** — ✅ zakres rozstrzygnięty 2026-08-28 (decyzja właściciela): **bez eksportu**. Jedyna forma eksportu, jaka miałaby sens, to integracja z kalendarzem telefonu (dodatkowe powiadomienie i przypomnienie o zadaniu) — pomysł zapisany w [[Projects/Backlog]], do pierwszej wersji zakładki nie wchodzi | `dev` + `designer` | design gotowy (`design_handoff_identyfikacja_v2`), czeka **wyłącznie na wdrożenie**; jeśli handoff zawiera element eksportu, przy wdrożeniu go pominąć |
