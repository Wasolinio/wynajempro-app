...
| **E7** | **Próg zwolnienia podmiotowego VAT + przegląd form opodatkowania** — zlecenie właściciela 2026-08-28 po prześwietleniu panelu pod zarzuty z naboru. **Po co:** panel ma udźwignąć zarzuty z komentarzy („gdzie VAT/ZUS/import usług") i objąć jak najszersze pole klientów. **Gotowe, gdy:** karta limitu 240 000 zł (art. 113) w panelu i eksporcie + raport form z rekomendacjami per forma. **Weryfikacja:** złote testy liczone ręcznie + przegląd `code-reviewer` z ręcznym przeliczeniem | `legal` + `dev` + `code-reviewer` | ✅ **KOD GOTOWY 2026-08-28** (commit `39f2795`; podatki 31/31, e2e 216/216, lint 0, build OK; ADR-026). ⏳ deploy za zgodą właściciela · decyzja: liniowy 19% (pierwszy kandydat rozbudowy przy popycie — raport `docs/legal/Formy-opodatkowania-wynajmu-2026-08-28.md`) |
