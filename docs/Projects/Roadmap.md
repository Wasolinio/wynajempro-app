...
| **F7** | ✅ **WYDANE 2026-08-26** — **purge `contact_messages` po 12 mies. od zakończenia korespondencji** (decyzja B-5): scheduler `cleanupContactMessages` (codziennie 03:45), koniec korespondencji = `max(createdAt, adminUpdatedAt)`. Commit `85bcb47`, merge + `firebase deploy --only functions` za zgodą właściciela; `functions:list` potwierdza trigger `scheduled`. Bramki: testy 41/41, e2e 204/204, przegląd `code-reviewer` (logika potwierdzona; finding o kłamiącej notce panelu naprawiony w `222c656`). 🔴 warunek publikacji Polityki **zszedł**. Szczegóły: [[Activity-Log]] 2026-08-26 | `dev` | ⏳ ogon: deploy `hosting:app` z poprawką notki panelu (`222c656`, w `main`) — osobna zgoda właściciela; do tego czasu panel pokazuje starą notkę „propozycja kierunkowa" |
| **F3** | **VAT, faktury, retencja dokumentów księgowych** | księgowy | 3 luki; zależne od formy działalności (nierejestrowana vs JDG) |
| **F4a** | 🔴 **Zniesienie poziomu 3 supportu — warunek publikacji** (decyzja 2026-08-26: Anthropic wykreślony z DPA, ale do końca bety kanał działa po staremu; z dniem publikacji dokumentów poziom 3 znika — inaczej DPA kłamie). Zmiana czysto proceduralna: `Proces-obslugi-zgloszen.md` + skill `zgloszenie` + CLAUDE.md mają gotowe adnotacje bramkowe do zdjęcia | właściciel ogłasza, agent nanosi | razem z F4 |
| **F4** | ✅ **KOD GOTOWY 2026-08-26** — generator `scripts/build-legal.mjs` (markdown → `src/data/legalDocs.js`, wzorzec centrum pomocy) + wspólny renderer `LegalDocPage.jsx`; trasy `/regulamin`, `/prywatnosc`, **`/dpa` (nowa)**; stare strony w `/_legacy`. **Filtr publikacyjny z bezpiecznikiem**: adnotacje robocze (kursywy z kluczami, znaczniki UZUPEŁNIENIE/PRZEGLĄD/KOREKTA, cytaty z odwołaniami do repo, emoji) nie przechodzą — build pada, gdy coś się prześlizgnie. Zweryfikowane w przeglądarce: 3 strony, tabele przewijane wewnętrznie na 375 px, zero treści roboczych. ⏸ **Zostaje deploy** (`firebase deploy --only hosting:app`, decyzja właściciela) — **w dniu deployu wpisać datę „Obowiązuje od" w trzech plikach źródłowych i przebiec `npm run legal:build`** (generator ostrzega, dopóki daty brak) — oraz bramka F4a |
