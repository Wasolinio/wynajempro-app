...
| **E3** *(2026-07-24)* | **Ostrzeżenie przed usunięciem konta** — dotyczy pytania **13** w sekcji 4.2 | Komunikat wyświetlany przed usunięciem konta wymieniał **węższy zakres** niż faktycznie wykonywana kasacja | Komunikat wymienia **pełny faktyczny zakres**: przewodniki wraz z danymi dostępowymi gości i zapisami akceptacji regulaminu, pliki, dane biznesowe, subskrypcję i rekord klienta Stripe, profil oraz konto logowania — ze wskazaniem nieodwracalności i braku okresu karencji. Zakres jest spójny z opisem usuwania danych w dokumencie nr 4. | Kod: `src/pages/dashboard/modals/AccountModal.jsx`, wydanie 2026-07-24 (commit `495aace`). **[AKTUALIZACJA 2026-08-13]** Komunikat został **obejrzany na produkcji** przez Operatora i odpowiada opisowi powyżej — wcześniejsze zastrzeżenie („potwierdzone wyłącznie w kodzie") jest nieaktualne. Przy tej samej weryfikacji potwierdzono działanie **ścieżki usunięcia konta założonego przez dostawcę zewnętrznego (Google)**, która realizuje art. 17 RODO dla kont bez hasła. Pytanie 13 **pozostaje otwarte** — prosimy oceniać brzmienie komunikatu |
...
| **E10** *(2026-08-13)* | **Weryfikacja adresu e-mail przy rejestracji** oraz **polityka haseł** — dotyczy dokumentu nr 4 (§9) i pytań o zabezpieczenia kont | Dokumenty opisywały wymóg weryfikacji adresu jako wdrożony, ale **pełnego przebiegu nie potwierdzono na żywo** — wykonawca nie mógł tego sprawdzić (wysyłka realnej wiadomości). Polityka haseł: minimum 6 znaków bez wymogu złożoności | **Weryfikacja adresu: potwierdzona na żywo 2026-08-13** — konto testowe, wiadomość, brak dostępu do panelu przed kliknięciem linku, dostęp po kliknięciu. Wymóg egzekwują równolegle aplikacja i reguły bazy danych, więc nie da się go obejść po stronie klienta; konta Google są zwolnione (adres potwierdzony u dostawcy). **Polityka haseł: Operator podjął 2026-08-13 decyzję o podniesieniu do 8 znaków z wymogiem litery i cyfry**; dotyczy wyłącznie nowych haseł, wprowadzenie w panelu dostawcy jeszcze przed nami — do czasu wprowadzenia obowiązuje stan dotychczasowy. **Prosimy o sygnał, czy przy profilu tej usługi rekomendują Państwo próg wyższy** | Weryfikacja Operatora 2026-08-13 (konto testowe na aliasie adresu). Opis: `Bezpieczenstwo-kont-i-danych.md` §9.1 i §9 |
| **E11** *(2026-08-17)* | **Polityka haseł — wprowadzona i egzekwowana** (uzupełnienie pozycji E10; dotyczy dokumentu nr 4 §9 oraz pytań o zabezpieczenia kont) | Decyzja Operatora z 2026-08-13 (8 znaków, litera i cyfra) była **podjęta, ale niewprowadzona** — obowiązywał stan dotychczasowy, czyli 6 znaków bez wymogów złożoności | **Wprowadzone 2026-08-17 w konsoli dostawcy, w trybie egzekwowania.** Wymagania: **minimum 8 znaków, wielka litera, mała litera i cyfra** — Operator zdecydował o progu **ostrzejszym** niż zapowiadany w E10 (doszedł wymóg wielkiej litery); znak specjalny nadal niewymagany. Obejmuje nowe hasła i ich zmiany; wymuszanie zmiany przy logowaniu pozostawiono wyłączone, więc istniejące konta nie są odcinane. Tego samego dnia zsynchronizowano warstwę aplikacji: przed zmianą ekran ustawiania nowego hasła po resecie deklarował „minimum 6 znaków", co po wprowadzeniu polityki wprowadzałoby użytkownika w błąd — dziś wymagania są widoczne przed wysłaniem formularza, a komunikat odmowy wskazuje brakujący element. **Pytanie z E10 pozostaje aktualne:** czy przy tym profilu usługi rekomendują Państwo próg wyższy lub sprawdzanie haseł znanych z wycieków (dostawca tego nie oferuje) | Ustawienie potwierdzone zrzutem panelu dostawcy (Operator, 2026-08-17); warstwa aplikacji zweryfikowana na produkcji tego samego dnia oraz testami `e2e/auth.spec.js`. Opis: `Bezpieczenstwo-kont-i-danych.md` §9.1 |
| **E12** *(2026-08-18)* | **Egzekwowanie App Check — zakres domknięty** (uzupełnienie E9; dotyczy dokumentu nr 4 §9.1 i pytania 4.1 pkt 5) | Egzekwowanie potwierdzone dla bazy danych i uwierzytelniania; **magazyn plików pozostawał w trybie monitorowania**, a status funkcji serwerowych opisano jako „bez egzekwowania" | **Magazyn plików: egzekwowanie włączone 2026-08-18** (przed zmianą 100% żądań zweryfikowanych; po włączeniu potwierdzono, że strony przewodników dla Gości działają wraz z materiałami). **Funkcje serwerowe: ustalenie faktyczne** — dostawca nie udostępnia przełącznika na poziomie usługi; ochrona jest ustawiana w kodzie każdej funkcji i obejmuje wszystkie funkcje wywoływane z aplikacji. Poza nią pozostają dwa wejścia wywoływane przez serwery podmiotów trzecich (płatności, udostępnianie kalendarza portalom), które z natury nie mogą przedstawić tokenu aplikacji. Stan końcowy: **baza danych, uwierzytelnianie i magazyn plików — egzekwowane** | Zmiana wykonana i zweryfikowana przez Operatora 2026-08-18. Opis: `Bezpieczenstwo-kont-i-danych.md` §9.1 |
| **E13** *(2026-08-18)* | **Osierocone pliki z przeszłości — usunięte** (dotyczy dokumentu nr 4: pozycja schodzi z §9 do §9.1) | Pliki przewodników usuniętych przed wdrożeniem kaskady czyszczącej (22.07) pozostawały w magazynie i były dostępne pod dawnymi adresami; pozycja figurowała na liście braków jako „wymaga jednorazowego czyszczenia" | **Wyczyszczone 2026-08-18: 15 plików w 8 przewodnikach (1,5 MB)** — okładki, załączone regulaminy obiektu i dwa artefakty testowe Operatora. Przebieg kontrolny po operacji: **zero** plików osieroconych. Zastosowane zabezpieczenia: przebieg próbny bez kasowania, ręczny przegląd listy przez Operatora, gwarda wieku 30 dni, sprawdzenie odwołań we wszystkich dokumentach bazy (zero) oraz kopia zapasowa plików przed usunięciem — magazyn plików nie jest objęty odtwarzaniem do punktu w czasie. Nowe przypadki są wykluczone mechanizmem kaskady wdrożonym 22.07 | Wykonane i zweryfikowane 2026-08-18. Opis: `Bezpieczenstwo-kont-i-danych.md` §9.1 |
| **E14** *(2026-08-22)* | **Warunki oferty „founding members" — uzupełnione** (dotyczy dokumentu nr 1 `Regulamin.md`, §6 ust. 5) | Ustęp brzmiał `[DO UZUPEŁNIENIA: warunki oferty dla uczestników bety — kto się kwalifikuje, wysokość i typ rabatu, okres obowiązywania, data zakończenia naboru]`. Pakiet przekazany Państwu 2026-07-22 **nie zawiera więc treści tej klauzuli** | **Warunki ustalone przez Operatora 2026-08-22 i wpisane do §6 ust. 5.** Użytkownik, który rozpocznie płatną Subskrypcję do dnia zakończenia naboru, **zachowuje cenę z dnia jej rozpoczęcia przez 12 miesięcy, liczonych od pierwszej płatności**; po tym okresie Subskrypcja odnawia się według ceny bieżącej, o czym Operator informuje **z co najmniej 30-dniowym wyprzedzeniem**, w trybie §10. Oferta nie wymaga kodu rabatowego ani odrębnego wniosku. ⏳ **Nieustalona pozostaje wyłącznie data zakończenia naboru** — jest pochodną daty uruchomienia sprzedaży, a ta zależy od Państwa akceptacji dokumentów. **Prosimy o dwie oceny:** **(1)** czy klauzula w tym brzmieniu jest wiążąca i wystarczająca wobec konsumenta, w szczególności czy mechanizm „cena z dnia rozpoczęcia przez 12 miesięcy, potem cena bieżąca" nie wymaga dodatkowego trybu obok §6 ust. 7 i §10; **(2)** czy **prezentowanie oferty jako „cena startowa gwarantowana na 12 miesięcy", a nie jako rabat procentowy, wyłącza obowiązek podania najniższej ceny z 30 dni przed obniżką** (dyrektywa Omnibus) — przy założeniu, że cena 29,99 zł nie była nigdy poprzedzona ceną wyższą. Od tej oceny zależy treść materiałów sprzedażowych | Decyzja Operatora z uzasadnieniem i odrzuconymi wariantami: `docs/Decisions.md` ADR-016. Treść klauzuli: `Regulamin.md` §6 ust. 5 |
