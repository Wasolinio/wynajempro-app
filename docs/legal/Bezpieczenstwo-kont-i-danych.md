...
> Prywatności i DPA §6.
> **Stan na:** 2026-08-13 (baza: stan produkcji z 2026-07-22, wydania z 2026-07-24, 2026-08-10 i 2026-08-13
> oraz weryfikacja przez Operatora w panelu dostawcy i w aplikacji z 2026-08-13).
...
> **zakres ostrzeżenia przy usuwaniu konta** (N6.2 — **od 2026-08-13 potwierdzony także na
> produkcji**, przez Operatora, wraz ze ścieżką usuwania konta założonego przez Google). Obie pozycje przeniesiono
...
| Wymagania wobec hasła | **Od 2026-08-17: minimum 8 znaków, w tym wielka litera, mała litera i cyfra** — polityka ustawiona u dostawcy w trybie egzekwowania („Require enforcement"), czyli rejestracja i zmiana hasła są odrzucane, dopóki hasło nie spełnia wymagań. Znak specjalny nie jest wymagany (decyzja Operatora). Wymóg dotyczy **nowych haseł i ich zmian**; wymuszanie zmiany przy logowaniu jest wyłączone, więc istniejące konta działają bez zmian. Aplikacja pokazuje wymagania **przed** wysłaniem formularza i przy odmowie wskazuje, którego elementu brakuje. Poprzednio: 6 znaków bez wymogów złożoności. | Konsola dostawcy (Authentication → Settings → Password policy); `src/utils/passwordPolicy.js`, `LoginPanel.jsx`, `AuthActionHandler.jsx` |
...
| **Sprawdzanie haseł znanych z wycieków** | Polityka haseł jest ustawiona i egzekwowana (patrz sekcja 9.1), ale dostawca nie oferuje w tym pakiecie porównywania haseł z bazami wycieków. Hasło zgodne z wymaganiami złożoności może więc być hasłem znanym publicznie. | Niska; do rozważenia razem z 2FA |
| **Eksport danych do arkusza (CSV)** | Nazwy Gości przy rezerwacjach pobieranych z portali rezerwacyjnych pochodzą z treści kalendarza dostawcy, czyli **spoza kontroli Operatora**, i trafiają do pliku CSV przekazywanego przez Gospodarza osobom trzecim (np. księgowemu). Wartość rozpoczynająca się od znaku działania bywa interpretowana przez arkusze kalkulacyjne jako formuła. Pozostałe elementy eksportu (kodowanie, separator, cytowanie pól) są poprawne. | Niska; naprawa techniczna prosta, odnotowana w rejestrze długu technicznego (2026-08-13) |
...
| **Ostrzeżenie przy usuwaniu konta** (N6.2) | Komunikat przed usunięciem konta wymienia **pełny faktyczny zakres kasacji**: przewodniki wraz z danymi dostępowymi gości (kody, WiFi) i podpisami akceptacji regulaminu, pliki, dane biznesowe (obiekty, rezerwacje, koszty, zadania), subskrypcję i rekord klienta Stripe, profil oraz konto logowania — z zastrzeżeniem, że operacja jest nieodwracalna i następuje bez okresu karencji. Zakres jest spójny z sekcją 8 (poprzednio komunikat wymieniał zakres węższy niż faktyczna kasacja). | Kod: `src/pages/dashboard/modals/AccountModal.jsx` (wydanie 2026-07-24). **Potwierdzone na produkcji 2026-08-13** przez Operatora (zrzut ekranu z aplikacji): komunikat wymienia pełny zakres kasacji zgodnie z opisem obok. Przy tej samej weryfikacji potwierdzono działanie **ścieżki usuwania konta założonego przez dostawcę zewnętrznego (Google)** — przycisk ponownego uwierzytelnienia; wcześniej mechanizm ten był potwierdzony wyłącznie w kodzie. Domyka to realizację art. 17 RODO także dla tej grupy kont. |
...
| **Weryfikacja adresu e-mail przy rejestracji** (N1) | Pełny przepływ **potwierdzony na żywo**: założenie konta → wiadomość weryfikacyjna → **brak dostępu do panelu przed kliknięciem linku** → dostęp po weryfikacji. Wymóg jest egzekwowany dwustronnie: przez aplikację i przez reguły bazy danych (`email_verified` w warunku dostępu), więc nie da się go obejść po stronie klienta. Konta zakładane przez dostawcę zewnętrznego (Google) są zwolnione z wymogu, ponieważ adres jest tam potwierdzony przy logowaniu. | Weryfikacja Operatora 2026-08-13 na koncie testowym założonym na osobny alias adresu. Wcześniej element ten był potwierdzony wyłącznie w kodzie i testach automatycznych — pełnego przebiegu (wysyłka realnej wiadomości → link → wejście) nie dało się sprawdzić w środowisku wykonawcy. |
| **Polityka haseł** | Wymagania podniesione i **realnie egzekwowane przez dostawcę**: minimum **8 znaków**, w tym **wielka litera, mała litera i cyfra**, tryb „Require enforcement" (próba założenia konta lub zmiany hasła niespełniającego wymagań jest odrzucana). Znak specjalny nie jest wymagany — decyzja Operatora, świadomie odrzucono wariant ostrzejszy. Wymóg obejmuje **nowe hasła i ich zmiany**; wymuszanie zmiany przy logowaniu pozostawiono wyłączone, więc istniejące konta nie są odcinane. Poprzedni stan: 6 znaków bez wymogów złożoności. | Ustawione przez Operatora w konsoli dostawcy **2026-08-17** (zrzut ekranu panelu). Tego samego dnia zsynchronizowano warstwę aplikacji: wymagania są pokazywane pod polem hasła przy rejestracji i przy ustawianiu nowego hasła po resecie, a komunikat odmowy wskazuje **brakujący element** (np. „brakuje: wielkiej litery"). Wcześniej ekran resetu deklarował 6 znaków, co po zmianie polityki wprowadzałoby użytkownika w błąd. Weryfikacja na produkcji 2026-08-17 (`wynajempro.com/login`) oraz testy `e2e/auth.spec.js`. **Zastrzeżenie:** dostawca nie porównuje haseł z bazami wycieków — pozycja pozostaje otwarta w sekcji 9. |
| **Osierocone pliki z przeszłości** (N6.5) | Pliki przewodników usuniętych **przed** wdrożeniem kaskady czyszczącej (22.07) nie były sprzątane i pozostawały dostępne pod dawnymi adresami. **Usunięte 2026-08-18: 15 plików w 8 przewodnikach (1,5 MB)** — okładki, załączone regulaminy obiektu oraz dwa artefakty testowe Operatora. Kontrolny przebieg po operacji wykazał **zero** plików osieroconych. Nowe przypadki są wykluczone mechanizmem kaskady, więc pozycja nie odtworzy się. | Skrypt jednorazowy uruchomiony przez Operatora (`functions/cleanup-orphan-guide-files-n6.cjs`), po przeglądzie kodu przez recenzenta. Zabezpieczenia zastosowane przed usunięciem: przebieg próbny bez kasowania, ręczny przegląd listy, gwarda wieku 30 dni (pominęła 0 plików), **kontrola odwołań we wszystkich dokumentach bazy** (18 dokumentów, 4 kolekcje — zero odwołań do usuwanych plików, także w postaci zakodowanej występującej w adresach pobierania) oraz **kopia zapasowa wszystkich 15 plików wykonana lokalnie przed operacją**, ponieważ magazyn plików nie jest objęty odtwarzaniem do punktu w czasie. Wynik: 15 usuniętych, 0 błędów, 1 plik z żywym rodzicem nietknięty. |
| **Egzekwowanie App Check** (N6.4) | Ochrona przed automatycznymi nadużyciami jest **realnie egzekwowana**, a nie tylko zainicjowana po stronie aplikacji. Stan per usługa: **baza danych (Cloud Firestore) — egzekwowanie włączone, 99% żądań zweryfikowanych**; **uwierzytelnianie — egzekwowanie włączone**; **magazyn plików — egzekwowanie włączone 2026-08-18** (przed zmianą tryb monitorowania ze 100% żądań zweryfikowanych; po włączeniu Operator potwierdził, że strony przewodników dla Gości otwierają się wraz z materiałami); funkcje serwerowe — egzekwowanie ustawiane **per funkcja w kodzie** i włączone dla wszystkich pięciu funkcji wywoływanych z aplikacji. Deklaracja z Polityki prywatności i DPA §6 odpowiada więc stanowi faktycznemu dla **bazy danych i logowania**, czyli warstw, w których przetwarzane są dane osobowe. | Potwierdzone 2026-08-13 odczytem panelu App Check w konsoli dostawcy (zrzut przekazany przez Operatora) oraz weryfikacją na żywo: po włączeniu egzekwowania dla uwierzytelniania strony gościa nadal działają. **[AKTUALIZACJA 2026-08-18]** Magazyn plików objęty egzekwowaniem, z weryfikacją wykonaną bezpośrednio po zmianie. W odniesieniu do funkcji serwerowych ustalono, że konsola dostawcy **nie oferuje przełącznika na poziomie usługi** — ochrona ustawiana jest w kodzie każdej funkcji i jest włączona dla wszystkich funkcji wywoływanych z aplikacji. Poza nią pozostają **dwa wejścia wywoływane przez serwery podmiotów trzecich** (obsługa płatności oraz udostępnianie kalendarza portalom rezerwacyjnym), które z natury nie mogą przedstawić tokenu aplikacji; ich zabezpieczenie opiera się na weryfikacji podpisu żądania i na braku danych osobowych w odpowiedzi. **Uwaga:** u dostawcy egzekwowanie dla uwierzytelniania jest oznaczone jako funkcja w wersji zapoznawczej (PREVIEW). |
...
Aktualizacja 2026-08-13 (po testach Operatora): do sekcji 9.1 doszły **ostrzeżenie przy usuwaniu konta**
(potwierdzone tym razem na produkcji, wraz ze ścieżką dla kont Google) oraz **weryfikacja adresu e-mail**
(pełny przebieg na żywo). Do listy braków dopisano **eksport CSV**. Wcześniej tego dnia: **egzekwowanie
App Check potwierdzone i przeniesione do sekcji 9.1** —
